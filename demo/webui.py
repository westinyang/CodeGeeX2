...
def main():
    ...
    with gr.Blocks(title="CodeGeeX2 DEMO") as demo:
        gr.Markdown(
            """
            <p align="center">
                <img src="file/resources/codegeex_logo.png">
            </p>
            """)
        gr.Markdown(
            """
        ...
        with gr.Row():
            with gr.Column():
                prompt = gr.Textbox(lines=14, placeholder='请输入描述或选择下面的示例输入',label='输入')
                with gr.Row():
                    gen = gr.Button("生成")
                    clr = gr.Button("清除")

            outputs = gr.Textbox(lines=15, label='输出')
        ...
        with gr.Row():
            with gr.Row():
                ...
                with gr.Row():
                    out_seq_length = gr.Slider(maximum=8192, value=4096, minimum=1, step=1, label='Output Sequence Length')
        ...
        with gr.Row():
            lang = gr.Radio(
                choices=["None"] + list(LANGUAGE_TAG.keys()), value='None', label='编程语言')
        ...
        gr_examples = gr.Examples(examples=examples, inputs=[prompt, lang],
                                  label="示例输入（单击可将示例插入到输入框中）",
                                  examples_per_page=20)
    if not args.auth:
        demo.launch(server_name=args.listen, server_port=args.port, share=False, inbrowser=True)
    else:
        demo.launch(server_name=args.listen, server_port=args.port, share=False, inbrowser=True, auth=(args.username, args.password))
